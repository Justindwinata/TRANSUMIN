...
void main() {
  group('RetryInterceptor', () {
    ...
    test('should identify retryable exception types', () {
      expect(interceptor.isRetryableException(DioExceptionType.connectionTimeout), true);
      expect(interceptor.isRetryableException(DioExceptionType.receiveTimeout), true);
      expect(interceptor.isRetryableException(DioExceptionType.sendTimeout), true);
      expect(interceptor.isRetryableException(DioExceptionType.connectionError), true);
      expect(interceptor.isRetryableException(DioExceptionType.cancel), false);
      expect(interceptor.isRetryableException(DioExceptionType.badResponse), false);
      expect(interceptor.isRetryableException(DioExceptionType.unknown), false);
    });

    test('should calculate exponential backoff delay', () {
      final interceptor = RetryInterceptor(maxRetries: 3, baseDelay: Duration(seconds: 1));
      
      expect(interceptor.calculateDelay(0), Duration(seconds: 1));  // 2^0 * 1s = 1s
      expect(interceptor.calculateDelay(1), Duration(seconds: 2));  // 2^1 * 1s = 2s
      expect(interceptor.calculateDelay(2), Duration(seconds: 4));  // 2^2 * 1s = 4s
      expect(interceptor.calculateDelay(3), Duration(seconds: 8));  // capped at maxRetries
    });
  });
}