...
class SavedJourneysScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SavedJourneysScreen> createState() =>
      _SavedJourneysScreenState();
}
...
class _SavedJourneysScreenState extends ConsumerState<SavedJourneysScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Perjalanan Tersimpan')),
      body:
          state.isLoading
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      subtitle: Text(
                        '${journey.originName} → ${journey.destName}',
                      ),
                      trailing: IconButton(
                        ...
                        onPressed:
                            () => _confirmDelete(
                              journey.id,
                              journey.label ?? journey.originName,
                            ),
                      ),
                      onTap: () {
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder:
                                (_) =>
                                    SavedJourneyReplanScreen(journey: journey),
                          ),
                        );
                      },
                    ),
    ...
    if (confirmed == true) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Perjalanan dihapus')));
      }
    }
  }
}
